...
test.afterEach.always(() => {
  ...
  loggerMock.getLogger().warn.resetHistory();
    ...
    t.true(
      loggerMock
        ...
        .debug.calledOnceWith(
          `New document (${newDocRef.id}) created in ${collectionName}`
        ),
...
test.serial(
  '"saveFirestoreDocument" adds a document to the specified collection without timestamp',
  async (t) => {
  ...
  async (t) => {
    ...
    t.true(
      firebaseAdminMock
        ...
        .collection()
        .doc()
        .update.calledOnceWith(newDocument)
    );
    ...
    t.true(
      loggerMock
        .getLogger()
        .debug.calledOnceWith(
          `Successfully updated document (${newDocRef.id}) in ${newDocRef.path}`
        ),
  ...
  async (t) => {
    ...
    t.true(
      firestoreCollectionMock()
        .where()
        .where()
        .limit.calledWith([], 0, firestoreCollectionMock)
    );
  ...
  async (t) => {
    ...
    t.true(
      firestoreCollectionMock()
        .where()
        .where()
        .limit.calledWith([], 0, firebaseAdminMock.getFirestore().collection)
    );
    t.true(firestoreCollectionMock().where().where().limit().get.calledOnce);
    t.true(loggerMock.getLogger().warn.calledOnce);
  }
);
